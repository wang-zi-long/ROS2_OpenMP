...
void dummy_load_100us_new_guard(rclcpp::GuardCondition::SharedPtr guard, int load_100us, bool use_openmp, int chain_idx, int node_idx, uint32_t data) {
  if(use_openmp){
    ...
    for (j = 0; j < (dummy_load_calib * load_100us /10); j++){
      for (i = 0 ; i < DUMMY_LOAD_ITER; i++) 
          __asm__ volatile ("nop");
      if(j == (dummy_load_calib * load_100us /10) / 2){
        guard->trigger();
      }
    }
  }
...
struct Sensor : public rclcpp::Node {
  Sensor(const std::string & node_name, const std::string & output_topic, uint32_t chain_idx, uint32_t node_idx, uint32_t exe_time_100us, std::chrono::duration<int,std::milli> period_ms, bool use_openmp, int priority)
  : Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(true)) {
    ...
    //   printf("|Tid:%ld|Sensor_guard_callback:%ld\n", tid, cur_time);
    // };
    // guard_condition_->set_on_trigger_callback(guard_callback);
    auto callback = [this, captured_pub, node_idx, chain_idx, exe_time_100us, use_openmp, priority]() -> void {
        ...
        dummy_load_100us_new_guard(this->guard_condition_, exe_time_100us, use_openmp, chain_idx, node_idx, msg->data);
...
struct Transfer : public rclcpp::Node {
  Transfer(const std::string & node_name, const std::string & input_topic, const std::string & output_topic, uint32_t chain_idx, uint32_t node_idx, uint32_t exe_time_100us, bool use_openmp, int priority)
  : Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(true)) {
    pub_ = this->create_publisher<std_msgs::msg::Int32>(output_topic, 10);
    std::weak_ptr<std::remove_pointer<decltype(pub_.get())>::type> captured_pub = pub_;
    guard_condition_ = std::make_shared<rclcpp::GuardCondition>();
    auto guard_callback = [&](size_t) {
      long int tid = syscall(SYS_gettid);
      uint64_t cur_time = get_clocktime();
      printf("|Tid:%ld|Transfer_guard_callback:%ld\n", tid, cur_time);
    };
    guard_condition_->set_on_trigger_callback(guard_callback);
    sub_ = this->create_subscription<std_msgs::msg::Int32>(
      input_topic,
      10,
      [this, captured_pub, node_idx, chain_idx, exe_time_100us, use_openmp, priority](std_msgs::msg::Int32::UniquePtr msg) {
...
struct Command : public rclcpp::Node {
  Command(const std::string & node_name, const std::string & input_topic, uint32_t chain_idx, uint32_t node_idx, uint32_t exe_time_100us, bool use_openmp, int priority)
  : Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(true)) {
    guard_condition_ = std::make_shared<rclcpp::GuardCondition>();
    auto guard_callback = [&](size_t) {
      long int tid = syscall(SYS_gettid);
      uint64_t cur_time = get_clocktime();
      printf("|Tid:%ld|Command_guard_callback:%ld\n", tid, cur_time);
    };
    guard_condition_->set_on_trigger_callback(guard_callback);
    sub_ = this->create_subscription<std_msgs::msg::Int32>(
      input_topic,
      10,
      [this, node_idx, chain_idx, exe_time_100us, use_openmp, priority](std_msgs::msg::Int32::UniquePtr msg) {
